...
class Adb(object):
        ...
        if not self.default_serial:
            ...
            if devices:
                if len(devices) is 1:
                    ...
                    raise EnvironmentError("Multiple get_devices attached but default android serial not set.")
            else:
                raise EnvironmentError("Device not attached.")
        return self.default_serial

    def devices(self):
        '''get a dict of attached get_devices. key is the device serial, value is device name.'''
        out = self.raw_cmd("get_devices").communicate()[0].decode("utf-8")
        match = "List of get_devices attached"
    ...
    def version(self):
        '''adb get_version'''
        match = re.search(r"(\d+)\.(\d+)\.(\d+)", self.raw_cmd("get_version").communicate()[0].decode("utf-8"))
