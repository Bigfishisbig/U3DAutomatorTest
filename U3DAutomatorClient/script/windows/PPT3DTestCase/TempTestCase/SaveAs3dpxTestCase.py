...
class SaveAs3dpxTestCase(Action, Operation, SystemDiaglog, TempOperation):
    ...
    def test_main(self):
        ...
        # subprocess.Popen(u'"D:\Program Files (x86)\PCPA ASSISTENT\AutoImport\ProAutoImport.exe" --STARTSCENE("打开多表格文件")')
        ...
        self.fileExist(save_path, "文件60s内另存为3dpx失败", 120)
        end_time = time.time()
        ...
        self.create()
        self.xls_getOldXls(SourcePath.File_Source + "Save3DPX_" + os.environ[Environment.StartTime] + ".xls")
        self.xls_write(str(start_time), str(end_time), spend_time)
        self.xls_close(SourcePath.File_Source + "Save3DPX_" + os.environ[Environment.StartTime] + ".xls")
