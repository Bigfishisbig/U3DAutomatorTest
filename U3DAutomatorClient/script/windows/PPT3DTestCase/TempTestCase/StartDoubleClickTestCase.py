...
class StartDoubleClickTestCase(Action, Operation, SystemDiaglog, TempOperation):
    '''双击启动3DPPT程序'''
    ...
    def test_main(self):
        ...
        self.s_doubleClickImg(SourcePath.File_Img_PPTXThumbnail, 5, "未找到pptx图标双击")
        # self.s_rightclickImg(SourcePath.File_Img_PPTXThumbnail, 5, "未找到pptx图标右键")
        # self.s_click (SourcePath.File_Img_RightStart, 5, "未找到打开3DPPT程序右键菜单")
        # time.sleep(5)
        ...
        self.create()
        self.xls_getOldXls(SourcePath.File_Source + "StartDoubleClick_" + os.environ[Environment.StartTime] + ".xls")
        self.xls_write(str(start_time), str(end_time), analyse_spend_time, rendering_spend_time)
        self.xls_close(SourcePath.File_Source + "StartDoubleClick_" + os.environ[Environment.StartTime] + ".xls")
